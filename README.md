[app.py](https://github.com/user-attachments/files/27017142/app.py)
...
if __name__ == "__main__":
    ...
    [requirements.txt](https://github.com/user-attachments/files/27017151/requirements.txt)
numpy>=1.26
pandas>=2.0
matplotlib>=3.8
scipy>=1.11
...
[calibration_current.csv](https://github.com/user-attachments/files/27017152/calibration_current.csv)





[fitted_params.json](https://github.com/user-attachments/files/27017156/fitted_params.json)

{
  ...
  "reference_concentrations": {
    "total_volume_ml": 15.3,
    "na2s2o3_m": 0.0037990196078431368,
    "ki_m": 0.06535947712418301,
    "fecl3_m": 0.0058823529411764705,
    "starch_g_l": 2.6143790849673203
  },
  "fixed_params": {
    "iodide_order": 2.0,
    "stoich_fe_per_s2o3": 1.0,
    "visual_i2_threshold_m": 1e-06,
    "starch_detection_order": 0.2,
    "davies_a_25c": 0.51,
    "davies_max_ionic_strength_m": 0.5,
    "no_color_time_s": 3600.0
  },
  "fit_params": {
    "rate_constant": 170.1035439067107,
    "lag_s": 3.242757390180105,
    "iodide_saturation_m_inv": 180.0,
    "ferric_depletion_alpha": 1.2847167581710248
  },
  "training_summary": {
    "rows": 24,
    "rmse_s": 3.9853247630647943,
    "mae_s": 3.025995868038244,
    "fitted_at_utc": "2026-04-21T03:32:55.227731+00:00",
    "columns": [
      "na2s2o3_ml",
      "ki_ml",
      "starch_ml",
    ...
    "ki_saturation_fitted": false,
    "note": "The model uses activity-corrected Fe3+/I- kinetics and explicit Fe3+ stoichiometric depletion. Add KI/FeCl3 scans to identify iodide saturation and ferric depletion outside this calibration set."
  }
}
...
[README.md](https://github.com/user-attachments/files/27017161/README.md)
# 碘钟反应 / 清一色控制预测器

这个项目用于预测 FeCl3-KI-Na2S2O3-淀粉体系的碘钟反应变色时间。

输入：

- 硫代硫酸钠 `Na2S2O3` 储备液浓度和体积
- 碘化钾 `KI` 储备液浓度和体积
- 淀粉 `starch` 浓度和体积
- 水 `water` 体积
- 触发剂三氯化铁 `FeCl3` 储备液浓度和体积

输出：

- 单次实验预测变色时间
- 拟合图 `fit_curve.png`
- 扫描图 `prediction_sweep.png`
- 本地交互界面
...
`calibration_current.csv` 的体积单位是 `mL`，时间单位是 `s`。

必需列：

```text
na2s2o3_ml,ki_ml,starch_ml,water_ml,fecl3_ml,time_s
```

储备液浓度列：

```text
na2s2o3_stock_m,ki_stock_m,fecl3_stock_m,starch_stock_g_l
```

前三个浓度单位是 `mol/L`，淀粉浓度单位是 `g/L`。

当前 `calibration_current.csv` 已录入截图中的有效实验点：

- FeCl3 = 0.5 M 组：9 个有效点
- FeCl3 = 0.3 M 组：15 个有效点
- 高硫代硫酸钠但未得到有效时间的点没有用于拟合

## 3. 重新拟合

```bash
python predictor.py fit --calibration calibration_current.csv --params-out fitted_params.json --plot-out fit_curve.png
```

当前结果：

```text
fit complete: rows=24 rmse_s=3.985 mae_s=3.026
```
...
如果 `S2O3^2-` 已经接近或超过 Fe3+ 可提供的氧化容量，界面会提示可能不显色或超过测量窗口。

## 4.1 临时公网分享

当前项目支持用 localtunnel 临时生成公网链接。运行：
...
把这个链接发给别人即可。只要这个 PowerShell 窗口和你的电脑不关，别人就能访问。

如果 localtunnel 页面要求输入 `Tunnel Password`，输入本机出口公网 IP。当前这次运行查到的是：

```text
116.169.80.153
```

注意：localtunnel 链接是临时链接，重启隧道后可能会变化。

## 5. 单点预测

截图中 FeCl3 = 0.3 M 组、Na2S2O3 = 4.75 mL 的条件：

```bash
python predictor.py predict --na2s2o3 4.75 --ki 5.0 --starch 4.0 --water 1.25 --fecl3 0.3 --params fitted_params.json
```

输出：

```text
predicted_time_s=62.164
```

## 6. 扫描 Na2S2O3

```bash
python predictor.py sweep --variable na2s2o3_ml --start 2.00 --stop 5.50 --points 200 --na2s2o3 4.75 --ki 5.0 --starch 4.0 --water 1.25 --fecl3 0.3 --params fitted_params.json --plot-out prediction_sweep.png
```

输出：

```text
sweep complete: points=200 monotonic_non_decreasing=True
start_prediction_s=8.274 stop_prediction_s=226.496
plot: prediction_sweep.png
```
...
```text
final concentration = stock concentration * added volume / total volume
```
...
- Fe3+ 不只是“触发剂”，而是生成 I2 的氧化剂
- S2O3^2- 会快速消耗 I2
- 每 1 mol S2O3^2- 大约对应消耗 1 mol Fe3+ 氧化当量
- 当 S2O3^2- 接近 Fe3+ 可提供的氧化容量时，剩余 Fe3+ 很低，变色时间会明显非线性拉长

速率部分采用：

```text
iodide_term = a(I-)^2 / (1 + K_I * a(I-))
```

含义：

- 低 KI 区接近二级 I-
- 高 KI 区逐渐饱和，不再无限按平方加速
- `a(I-)` 使用活度修正，而不是直接使用名义浓度

Fe3+ 耗尽部分采用：

```text
X = required_Fe3+ / initial_Fe3+
depletion_factor = ((1 / (1 - X))^alpha - 1) / alpha
```

当 `X` 接近 1 时，时间会快速上升；这正是截图中硫代硫酸钠增加后时间明显上翘的主要原因。

## 8. 后续提高准确度

现在模型已经能捕捉 Na2S2O3 增加时的非线性拉长。  
若要继续提高 KI、FeCl3 变化条件下的准确度，需要补充：
...
```
    ...
    [predictor.py](https://github.com/user-attachments/files/27017146/predictor.py)
